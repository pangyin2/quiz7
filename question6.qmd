...
race <- sample(c("White", "Black", "Other"), size = n, replace = TRUE, prob = c(0.7, 0.2, 0.1))
gender <- sample(c("Male", "Female"), size = n, replace = TRUE, prob = c(0.5, 0.5))

# Dependent variable (vote_preference) with some imperfect relationship
# Assuming that 'White' or 'Male' are slightly more likely to prefer 'Candidate_A'
vote_preference <- ifelse(race == "White" & gender == "Male", 
                          sample(c("Candidate_A", "Candidate_B"), size = n, replace = TRUE, prob = c(0.6, 0.4)),
                          ifelse(race == "White" & gender == "Female",
                                 sample(c("Candidate_A", "Candidate_B"), size = n, replace = TRUE, prob = c(0.55, 0.45)),
                                 ifelse(race == "Black" | race == "Other",
                                        sample(c("Candidate_A", "Candidate_B"), size = n, replace = TRUE, prob = c(0.4, 0.6)),
                                        sample(c("Candidate_A", "Candidate_B"), size = n, replace = TRUE, prob = c(0.5, 0.5))
                                 )
                          )
)
